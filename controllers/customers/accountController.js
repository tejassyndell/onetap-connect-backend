...
exports.getActiveAccounts = catchAsyncErrors(async (req, res, next) => {
  try {
    ...
    const decodedIds = values.map(
      (token) => jwt.verify(token, process.env.JWT_SECRET).userID
    );
    const userPromises = decodedIds.map((id) =>
      User.findById(id).select("first_name last_name email avatar")
    );

...
const CompanyShareReferralModel = require("../../models/NewSchemas/Company_Share_Referral_DataModel");
...
exports.googleLogin = catchAsyncErrors(async (req, res, next) => {
  ...
  if (user.delete_account_status === "inactive") {
    return next(
      new ErrorHandler(
        "Your account has been deleted, please check your Email for more information.",
        401
      )
    );
  }
...
exports.login = catchAsyncErrors(async (req, res, next) => {
  ...
  if (user.googleId !== null) {
    return next(
      new ErrorHandler(
        "This account is associated with a Google login. Please log in with Google.",
        400
      )
    );
  }
...
exports.requestToManagerForUpdateUserInfo = catchAsyncErrors(
  async (req, res, next) => {
...
exports.inviteTeamMember = catchAsyncErrors(async (req, res, next) => {
  const { memberData, manage_superadmin, manager_firstname, manager_email } =
    req.body;
  const { companyID } = req.user;
...
exports.inviteTeamMemberByCSV = catchAsyncErrors(async (req, res, next) => {
  ...
  for (const userData of existingMails) {
    ...
    <body style="margin: 0; line-height: normal; font-family: 'Assistant', sans-serif;">
      ...
      const companyuniqueSlugValue =
        existingCompanySlug?.companyunique_slug[0]?.value;
      ...
      const user_parmalink = await parmalinkSlug.create({
        ...
        unique_slugs: [{ value: generatedCode, timestamp: Date.now() }],
        companyunique_slug: [
          { value: companyuniqueSlugValue, timestamp: Date.now() },
        ],
        userurlslug: generatedCode,
...
exports.updateCompanySlug = catchAsyncErrors(async (req, res, next) => {
  ...
  try {
    ...
    const updatedCompanyinsluddata = await parmalinkSlug.updateOne(
      { user_id: userid },
      {
        $addToSet: { companyunique_slug: uniquecompanySlug },
        companyurlslug: trimslug,
      }
    );
    if (!updatedCompany) {
...
exports.checkoutHandler = catchAsyncErrors(async (req, res, next) => {
  ...
  if (saveAddress) {
    if (selectedEditAddress) {
      const index = shippingAddressFind.shipping_address.findIndex(
        (address) =>
          address._id.toString() === selectedEditAddress._id.toString()
      );
      if (index !== -1) {
  ...
  let companyVar = null;
  if (companyslug !== null) {
    const iscompanyDuplicate = await parmalinkSlug.exists({
      "companyunique_slug.value": companyslug,
    });
  ...
  await sendOrderConfirmationEmail(
    ...
    planData.renewal_date,
    planData.recurring_amount,
...
`,
    ...
    res.status(200).json({
      success: false,
      msg: "Error sending order confirmation email",
      error,
    });
  }
}
  ...
  try {
    ...
    if (slug !== null) {
      ...
      if (!isDuplicate) {
        ...
        await User.updateOne({ _id: user._id }, { userurlslug: slug });
      }
    }
...
exports.updateUserPlanonRoleChange = catchAsyncErrors(
  async (req, res, next) => {
...
exports.guestcheckoutHandler = catchAsyncErrors(async (req, res, next) => {
  const { userData, billingdata, shippingData, shipping_method, cardDetails } =
    req.body;
  ...
  res
    .status(201)
    .json({ success: true, message: "Guest customer created successfully" });
});
...
exports.createOrder = catchAsyncErrors(async (req, res, next) => {
  try {
    ...
    const { smartAccessories, totalAmount, tax } = req.body;
...
exports.generateotp = catchAsyncErrors(async (req, res, next) => {
  ...
  const verificationToken = jwt.sign({ otp, email }, process.env.JWT_SECRET, {
    expiresIn: "10m",
  });
  try {
...
exports.verifyotp = catchAsyncErrors(async (req, res, next) => {
  ...
  try {
    ...
    if (otp === userotp) {
      const recoveryToken = jwt.sign({ _id: userid }, process.env.JWT_SECRET, {
        expiresIn: "7d",
      });
...
async function handleDataDeletionSuccess(companyid) {
  ...
  try {
    ...
    for (const user of companyUsers) {
      ...
      const userInfoDeletePromise = UserInformation.findOneAndDelete({
        user_id: user._id,
      });
      pushDeletePromise(
        userInfoDeletePromise,
        `User Information for _id ${user._id} not found`
      );
      const cardsDeletePromise = Cards.findOneAndDelete({ userID: user._id });
      pushDeletePromise(
        cardsDeletePromise,
        `Card info for _id ${user._id} not found`
      );
      const billingAddressDeletePromise = billingAddress.findOneAndDelete({
        userId: user._id,
      });
      pushDeletePromise(
        billingAddressDeletePromise,
        `Billing address for _id ${user._id} not found`
      );
      const shippingAddressDeletePromise = shippingAddress.findOneAndDelete({
        userId: user._id,
      });
      pushDeletePromise(
        shippingAddressDeletePromise,
        `Shipping address for _id ${user._id} not found`
      );
    }
    await Promise.all(deletePromises);
  } catch (error) {}
      ...
      async (err, decoded) => {
        ...
        try {
          ...
          return res.status(200).json({
            success: true,
            message: "Token verified and fields updated successfully",
          });
        } catch (error) {
          return res
            .status(500)
...
exports.google_verify_recover_account = catchAsyncErrors(
  async (req, res, next) => {
...
exports.accountSetupsteps = catchAsyncErrors(async (req, res, next) => {
  ...
  try {
    ...
    res
      .status(200)
      .json({ message: "Account_setup updated successfully", user });
  } catch (error) {
    res.status(500).json({ message: "Internal Server Error" });
  }
});
...
exports.redirectUser = catchAsyncErrors(async (req, res, next) => {
  const { slug } = req.body;
  try {
    const permalink = await parmalinkSlug.findOne({
      "unique_slugs.value": slug,
    });
    ...
    if (permalink.isactive) {
      const latestSlug = await parmalinkSlug.findOne({
        user_id: permalink.redirectUserId,
      });

      if (latestSlug) {
        return res.status(200).json({
          success: true,
          slug: latestSlug.unique_slugs[latestSlug.unique_slugs.length - 1]
            .value,
        });
      } else {
        return res
      ...
      if (
        permalink.unique_slugs.length === 1 ||
        slug === permalink.unique_slugs[0].value
      ) {
        return res
          .status(200)
          .json({ success: true, slug: permalink.unique_slugs[0].value });
      }

      if (permalink.unique_slugs.length > 1) {
        return res.status(200).json({
          success: true,
          slug: permalink.unique_slugs[permalink.unique_slugs.length - 1].value,
        });
      }
    }
  ...
  transporter.sendMail(mailOptions, async function (error, info) {
    if (error) {
      ...
      try {
        ...
        res
          .status(200)
          .json({ message: "Form data saved successfully. Email sent." });
      } catch (dbError) {
...
exports.postshipstation = catchAsyncErrors(async (req, res, next) => {
  ...
  const xmlOrders = allorders
    .map((order) => {
      const orderID = order._id;
      const odrNum = order.orderNumber;
      const odrtotalweight = order.sumTotalWeights;
      const OrderDate = order.createdAt;
      const formattedOrderDate = `${(OrderDate.getMonth() + 1)
        .toString()
        .padStart(2, "0")}/${OrderDate.getDate()
        .toString()
        .padStart(2, "0")}/${OrderDate.getFullYear()} ${OrderDate.getHours()
        .toString()
        .padStart(2, "0")}:${OrderDate.getMinutes()
        .toString()
        .padStart(2, "0")} ${OrderDate.getHours() >= 12 ? "PM" : "AM"}`;
      const OrderStatus = order.status;
      const LastModified = order.updatedAt;
      const formattedLastDate = `${(LastModified.getMonth() + 1)
        .toString()
        .padStart(2, "0")}/${LastModified.getDate()
        .toString()
        .padStart(
          2,
          "0"
        )}/${LastModified.getFullYear()} ${LastModified.getHours()
        .toString()
        .padStart(2, "0")}:${LastModified.getMinutes()
        .toString()
        .padStart(2, "0")} ${LastModified.getHours() >= 12 ? "PM" : "AM"}`;
      ...
      const xmlItems = order?.smartAccessories
        .map((item) => {
...
exports.getrateoforder = catchAsyncErrors(async (req, res, next) => {
  ...
  axios
    .post(ratesEndpoint, shipmentData, {
      headers: {
...
exports.sendTestData = async (req, res, next) => {
  try {
    const latestUsers = await User.find(
      {},
      "first_name last_name email address _id, avatar"
    )
      .sort({ createdAt: -1 })
      .limit(5);
...
exports.getorderdetails = catchAsyncErrors(async (req, res, next) => {
  try {
    ...
    if (!orderNumber || !user) {
      return res.status(400).json({
        error: "Both orderNumber and userId are required in the request body",
      });
    }
...
exports.getAddonsForOrderSummary = catchAsyncErrors(async (req, res, next) => {
  try {
    ...
    const addonDetails = await Adminaddonsschema.find({
      _id: { $in: addonIds },
    });
...
exports.verifydeactivateAccountotp = catchAsyncErrors(
  async (req, res, next) => {
    const { email, otp } = req.body;
    const user = await User.findOne({ email: email });
    const storedotp = user.otptoken;
...
exports.assignSmartAccessroiesToUser = catchAsyncErrors(
  async (req, res, next) => {
    try {
...
exports.updateSmartAccessoryStatus = catchAsyncErrors(
  async (req, res, next) => {
    try {
...
exports.removeUserFromSmartAccessories = catchAsyncErrors(
  async (req, res, next) => {
    try {
...
exports.getUserAssignSmartAccessoriesForCompany = catchAsyncErrors(
  async (req, res, next) => {
    try {
      ...
      if (!SmartAccessorie) {
        return res
          .status(404)
          .json({ message: "SmartAccessorie does not have a data" });
      }

      const smartAccessories = SmartAccessorie.filter(
        (sa) => sa.userId !== undefined
      );
      const smartAccessorieswithoutUserId = SmartAccessorie.map((e) => e);

      res.status(200).json({
        smartAccessories,
        smartAccessorieswithoutUserId,
        AllSmartAccessories: SmartAccessorie,
      });
    } catch (error) {
      next(error); // Pass the error to the error handling middleware
    }

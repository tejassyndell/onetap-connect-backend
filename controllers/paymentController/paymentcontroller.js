...
const monthlyProfessionalPriceID =
  process.env.MONTHLY_PROFESSIONAL_PLAN_PRICE_ID;
  ...
  try {
    ...
    const productID =
      type === "monthly"
        ? planName === "Professional"
          ? Product_Professional_monthly
          : Product_Team_monthly
        : planName === "Professional"
        ? Product_Professional_Yearly
        : Product_Team_Yearly;
    ...
    if (isCouponApplied) {
      const couponOptions = {
        ...
        duration_in_months:
          typess === "monthly"
            ? couponData.xpayments
            : couponData.xpayments * 12,
        applies_to: {
          products: [productID],
        },
        currency: "usd",
    ...
    const phases = [
      {
        ...
        default_payment_method: selectedCard
          ? paymentToken
    ...
    if (latestInvoice.payment_intent) {
      ...
      return res.status(200).json({
        success: true,
        client_secret: subscriptionPaymentIntetn.client_secret,
        subscriptionID: subscription.id,
        status: subscriptionPaymentIntetn.status,
        endDate: subscription.current_period_end,
        addonClient_secret: paymentIntent && paymentIntent.client_secret,
        addonstatus: paymentIntent && paymentIntent.status,
      });
    }

    return res.status(200).json({
      success: true,
      client_secret: "switch-plan",
      subscriptionID: myPayment.id,
      status: "active",
      endDate: myPayment.current_period_end,
      addonClient_secret: paymentIntent && paymentIntent.client_secret,
      addonstatus: paymentIntent && paymentIntent.status,
    });
  } catch (error) {
    ...
    try {
      const deleteCustomer =
        customerID && (await stripe.customers.del(customerID));
  ...
  try {
    if (type === "cancel") {
      ...
      res.status(200).json({
        success: true,
        message:
          "Switched to manual renewal. Invoices will be sent for manual payment.",
      });
    } else if (type === "enable") {
      await stripe.subscriptions.update(subscription_id, {
        collection_method: "charge_automatically",
      ...
      res
        .status(200)
        .json({ success: true, message: "Switched to automatic renewal." });
    }
  } catch (error) {
    console.error(error);
    res.status(500).json({ success: false, error: error.message });
...
exports.cancelPlan = catchAsyncErrors(async (req, res, next) => {
  try {
    ...
    if (!subId) {
      return res
        .status(500)
        .json({ success: false, error: "No Subscription Id found" });
    }
    ...
    if (!canceledSubscription) {
      return res
        .status(500)
        .json({ success: false, error: "Error while canceling subscription" });
    }
    ...
    const updatedUserInfo = await UserInformation.findOneAndUpdate(
      { "subscription_details.customer_id": canceledSubscription.customer },
      {
        $set: {
          ...
          "subscription_details.total_user": [
            { baseUser: 1, additionalUser: 0 },
          ],
    ...
    res
      .status(200)
      .json({ success: true, delete: "Subscription Canceled successfully" });
  } catch (error) {
    console.error(error);
    res.status(500).json({ success: false, error: error.message });
  }
...
exports.cancelPlandeactivateaccount = catchAsyncErrors(
  async (req, res, next) => {
    try {
...
exports.isActive = catchAsyncErrors(async (req, res, next) => {
  try {
    ...
    const subscription = await stripe.subscriptions.retrieve(
      userInformation.subscription_details.subscription_id
    );
    if (!subscription) {
...
exports.createOrderWithoutPayment = catchAsyncErrors(async (req, res, next) => {
  try {
    ...
    if (userId !== "Guest") {
      ...
      let billingAddressFind = await billingAddressModal.findOne({
        userId: user._id,
      });
      ...
      let shippingAddressFind = await shippingAddressModal.findOne({
        userId: user._id,
      });
...
exports.createOrderWithoutPaymentAndSendInvoice = catchAsyncErrors(
  async (req, res, next) => {
    try {
...
exports.createOrder = catchAsyncErrors(async (req, res, next) => {
  try {
    ...
    let attachedPaymentMethod;
    if (!selectedCard && existingcard === false) {
      attachedPaymentMethod = await stripe.paymentMethods.attach(
    ...
    if (paymentIntent) {
      ...
      const order = new Order({
        ...
        isGuest: userId === "Guest" ? true : false,
        userShippingOrderNote:
          userData?.userShippingOrderNote === undefined
            ? ""
            : userData?.userShippingOrderNote,
        referredby:
          userData?.referredby === undefined ? "" : userData?.referredby,
...
async function sendpurchaseOrderconfirmationEmail(
  customeremail,
  shippingAddress,
  smartAccessories,
  order
...
exports.updateCustomerCreditBalance = catchAsyncErrors(
  async (req, res, next) => {
  ...
  try {
    ...
    const updatedUserInformation = await UserInformation.updateMany(
      { company_ID: companyID, "subscription_details.plan": { $ne: null } },
      {
        $push: {
          "subscription_details.addones": {
            $each: addaddons.map((addon) => addon),
          },
        },
...
exports.addonPurchase = catchAsyncErrors(async (req, res, next) => {
  try {
    ...
    try {
      ...
      console.error("Cleanup performed after failure:", detachPT);
    } catch (cleanupError) {
      console.error("Error during cleanup:", cleanupError);
    }
    res.status(500).json({ success: false, error: error.message });
...
exports.purchaseusers = catchAsyncErrors(async (req, res, next) => {
  try {
    ...
    const updatedUserInformation = await UserInformation.updateMany(
      ...
      {
        $inc: {
          "subscription_details.total_user.0.additionalUser":
            addusers.addusercount,
...
exports.fetchCardsforOtcAdminPanel = catchAsyncErrors(
  async (req, res, next) => {
    const { customerID } = req.body;
...
exports.updateCardsforOtcAdminPanel = catchAsyncErrors(
  async (req, res, next) => {
...
exports.switchToManualRenewalforOtcAdminPanel = catchAsyncErrors(
  async (req, res, next) => {
    const { subscription_id, userId, type } = req.body.userData;
...
exports.createAdminPlanOrder = catchAsyncErrors(async (req, res, next) => {
  try {
    ...
    if (smartAccessories.length > 0) {
      productsInvoice = await createInvoiceItems(
        smartAccessories,
        customerID,
        "product",
        "txcd_99999999"
    ...
    if (oneTimeAddons.length > 0) {
      onetimeAddonsInvoice = await createInvoiceItems(
        oneTimeAddons,
        customerID,
        "addon"
      );
    ...
    if (isCouponApplied) {
      ...
      const matchedCouponsData = matchCouponsWithItems(
        [coupon2],
        separatedItems
      );
      ...
      const matchedCodes = matchItemIDWithLineItems(
        matchedCouponsData,
        invoice.lines.data
      );
        ...
        for (const key in matchedCodes) {
          for (const item of matchedCodes[key]) {
            try {
              ...
              console.error(
                `Error applying discount for item ${item.id}:`,
                error
              );

      ...
      const updatedCreditbalance = await UserInformation.findOneAndUpdate(
        {
          "subscription_details.customer_id":
            balanceTransactions.data[0].customer,
        },
        {
          $set: {
            "subscription_details.creditBalance":
              balanceTransactions.data[0].ending_balance / 100,
          },
        },
        { new: true }
      );
...
async function upComingInvoiceUpdate(upComingInvoiceData) {
  try {
    // NOT DUMMY NEED TO USE THIS FUNCTION, AFTER CERTAIN FUNCTIONALITIES ARE COMPLETED
    return null;
  } catch (error) {
    throw error;
  }
